...
let setup_common_context ctx com =
	...
	Common.define_value com Define.SourceHeader ("Generated by Haxe " ^ (s_version true));
...
and init ctx =
	let usage = Printf.sprintf
		...
		s_version_with_build (if Sys.os_type = "Win32" then ".exe" else "")
	in
	let com = ctx.com in
...
try
	...
	let basic_args_spec = [
		...
		("Miscellaneous",["--version"],["-version"],Arg.Unit (fun() ->
			com.info (s_version_with_build) null_pos;
			did_something := true;
